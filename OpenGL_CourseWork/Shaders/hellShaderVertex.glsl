...
void main(void) {
	...
	OUT.normal = normalize(normalMatrix * normalize(oNormal));
	OUT.tangent = normalize(normalMatrix * normalize(oTangent));
	OUT.binormal = normalize(cross(OUT.normal, OUT.tangent));

	OUT.worldPos = (modelMatrix * vec4(vertPos.xyz, 1)).xyz;
	// New !
	OUT.shadowProj = (textureMatrix * vec4(vertPos.xyz + (oNormal * 1.5), 1));
